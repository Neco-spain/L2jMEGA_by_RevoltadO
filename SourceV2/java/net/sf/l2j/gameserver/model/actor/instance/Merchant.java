...
public class Merchant extends Folk
{
			...
			if(Config.ENABLE_VISUAL_SET)
			{
				...
				if (Config.ENABLE_VISUAL_EFFECT)
				{
					...
					if (skill != null)
					{
						...
						player.broadcastPacket(new SocialAction(player, Config.ID_VISUAL_SOCIAL_ACTION));
						ThreadPool.schedule(() -> player.setVisualTest(0), 1000 * Config.VISUAL_TEST_SEGUNDS);
						sendVisualTime(player);
						if (Config.CAMERA_ENABLED)
						{
						player.sendPacket(new SpecialCamera(player.getObjectId(), Config.CAMERA_DISTANCE, Config.CAMERA_POV, Config.CAMERA_ANGLE, Config.CAMERA_SPEED, Config.CAMERA_DELAY));
						}else {
	...
	private static void sendVisualTime(Player player)
	{
		player.sendMessage("Visual Time: " + Config.VISUAL_TEST_SEGUNDS + " seconds");
	}
}