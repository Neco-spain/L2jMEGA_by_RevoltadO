...
public class Zaken extends L2AttackableAIScript
{
	...
	public Zaken()
	{
		...
		ThreadPool.scheduleAtFixedRate(new Runnable()
		{
			...
			public void run()
			{
				if (GetTimeHour() == Config.ZAKEN_ENABLE_DOOR)
				{
	...
	public String onSpellFinished(Npc npc, Player player, L2Skill skill)
	{
		if (npc.getNpcId() == ZAKEN)
		{
			...
			else if (skillId == MASS_TELEPORT)
			{
				...
				for (Creature character : npc.getKnownType(Monster.class))
					if (character != player && !MathUtil.checkIfInRange(250, player, character, true))
					{
						int r1 = Rnd.get(15);
