...
public class MapRegionTable
{
	...
	public Location getLocationToTeleport(Creature character, TeleportType teleportType)
	{
		...
		if (castle != null && castle.getSiege().isInProgress())
			...
			// Check if player is in FlagZone.
			final L2FlagZone flag = ZoneManager.getInstance().getZone(player, L2FlagZone.class);
			if (flag != null)
				return flag.getSpawnLoc();

			// Check if player is in random zone.
			final L2CustomZone random = ZoneManager.getInstance().getZone(player, L2CustomZone.class);
			if (random != null)
