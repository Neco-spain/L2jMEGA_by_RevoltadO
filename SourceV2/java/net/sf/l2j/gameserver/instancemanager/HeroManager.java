...
public class HeroManager
{
	...
	protected HeroManager()
	{
		...
		_scheduler = ThreadPool.scheduleAtFixedRate(new HeroTask(), 1000L, 1000L);
		load();
	}
	...
	public void load()
	{
		try
		{
			...
			try
			{
				...
				while (rs.next())
				{
					_heros.put(Integer.valueOf(rs.getInt("objectId")), Long.valueOf(rs.getLong("duration")));
				}
				rs.close();
				statement.close();
			}
	...
	{
		_heros.put(Integer.valueOf(objectId), Long.valueOf(duration));
		_herosTask.put(Integer.valueOf(objectId), Long.valueOf(duration));
	...
	{
		_heros.remove(Integer.valueOf(objectId));
		_herosTask.remove(Integer.valueOf(objectId));
	...
	{
		return _heros.containsKey(Integer.valueOf(objectId));
	}
	
	public long getHeroDuration(int objectId)
	{
		return _heros.get(Integer.valueOf(objectId)).longValue();
	}
